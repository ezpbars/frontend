...
export class ProgressBarTraceView {
    /**
     * 
     * @param {import("/js/app/progress_bars/traces/progress_bar_trace.js").ProgressBarTrace} progresBarTrace the progress bar trace to show
     * @param {function() : any} onDelete the function to call after the progress bar trace is deleted
     */
    constructor(progresBarTrace, onDelete) {
        /**
    ...
    async _onDelete() {
        const response = await fetch(
            apiUrl("/api/1/progress_bars/traces/?" + new URLSearchParams({ pbar_name: this.progressBarTrace.get("progressBarName"), trace_uid: this.progressBarTrace.get("uid") })),
    ...
    render() {
        ...
        this.element.appendChild((() => {
            ...
            const data = this.progressBarTrace;
            return (new ResourceSection(data, "progressBarName", { formatter: (progressBarName) => progressBarName })).element;
        })());
        this.element.appendChild((() => {
