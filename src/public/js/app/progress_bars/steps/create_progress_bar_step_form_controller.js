...
import { ITERATED_TECHNIQUE_NATIVE_TO_API, ONE_OFF_TECHNIQUE_NATIVE_TO_API, parseProgressBarStep } from "/js/app/progress_bars/steps/progress_bar_step.js";
    ...
    render() {
        ...
        this.element.appendChild((() => {
            ...
            form.addEventListener("submit", async (e) => {
                ...
                try {
                    const response = await fetch(
                        apiUrl("/api/1/progress_bars/steps/?" + new URLSearchParams({ pbar_name: progressBar.value.get("name"), step_name: name.value }).toString()),
                        AuthHelper.auth({
                            method: "POST",
                            headers: { "content-type": "application/json; charset=UTF-8" },
                            body: JSON.stringify({
            ...
            form.appendChild((() => {
                const formGroup = new FormGroup((() => {
                    const search = new SearchController("name", new ProgressBarReader(), (f) => f.name, (f, v) => Object.assign({}, f, { name: v }));
                ...
                const collapse = new Collapse((() => {
                    ...
                    iterated.addListenerAndInvoke((iterated) => {
                        if (iterated === null) {
                            ...
                            formGroup.help.value = "this step is repeated, potentially a different number of times, every trace";
                        } else {
            ...
            form.appendChild((() => {
                const collapse = new Collapse((() => {
                    const formGroup = new FormGroup((() => {
                        ...
                        select.addEventListener("change", (e) => {
                            const s = select.value;
                            if (s !== "percentile" && s !== "harmonicMean" && s !== "geometricMean" && s !== "arithmeticMean") {
            ...
            form.appendChild((() => {
                const collapse = new Collapse((() => {
                    ...
                    return formGroup.element;
                })(), { visible: progressBar.value !== null && iterated.value === "oneOff" && oneOffTechnique.value === "percentile" });
                const updateVisibility = () => {
                    collapse.visible.value = progressBar.value !== null && iterated.value === "oneOff" && oneOffTechnique.value === "percentile";
                };
                progressBar.addListener(updateVisibility);
                iterated.addListener(updateVisibility);
                ...
                const collapse = new Collapse((() => {
                    ...
                    const updateHelp = () => {
                        ...
                        if (technique === "bestFit.linear") {
                            formGroup.help.value = "solves for b, m in y = mx+b where x is the numebr of iterations and y is the time the step took via least squares regression";
            ...
            form.appendChild((() => {
                const collapse = new Collapse((() => {
                    ...
                    return formGroup.element;
                })(), { visible: progressBar.value !== null && iterated.value === "iterated" && iteratedTechnique.value === "percentile" });
                const updateVisibility = () => {
                    collapse.visible.value = progressBar.value !== null && iterated.value === "iterated" && iteratedTechnique.value === "percentile";
                };
                progressBar.addListener(updateVisibility);
                iterated.addListener(updateVisibility);
                oneOffTechnique.addListener(updateVisibility);
